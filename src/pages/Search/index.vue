...
export default {
  ...
  methods:{
    ...
    removeCategoryName() {
      ...
      this.getData()
      // 地址栏也需要更改：进行路由跳转（现在路由只跳转到自己这里）
      // 删除query参数，如果有params参数，不应该删掉；
    ...
    attrInfo(attr,attrValue) {
      // 整理参数格式
      let props= `${attr.attrId}:${attrValue}:${attr.attrNmae}`;
  ...
  watch:{
    ...
    $route(newValue,oldValue) {
      // 每一次请求完毕，应该把相应的123级分类的id置空，方便接受下一次的新的id
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      // 再次发送请求之前，整理带给服务器的参数
